...
2. Survivor expeditions - EXPANDED SCOPE (owner, July 12, 2026): now a three-phase system requiring a written spec before code. Phase E1: groups of 2-4 from the real roster, per-site durations and difficulty tiers, Flux launch costs, reward scaling, and planetary surveys as a departure requirement from Cinder onward (each world's continuity checklist requires N completed surveys to certify the world safe). Outcomes: success / lean return / setback (wounded). Phase E2: per-survivor health; wounded crew occupy medical life-support capacity and recover over time with Doctors accelerating recovery; armor and weapons crafted at the Foundry, unlocked by the Threat Operations research branch. Phase E3: mortality with agency - failed expeditions down crew (recoverable via rescue expeditions); death is permanent only when the player chooses to abandon them, recorded in persistent memorials; explicitly-labeled hazardous tiers may carry true death risk, opt-in only. Core rule preserving the idle contract: crew NEVER die from RNG alone, offline, or by default - loss always requires an explicit player choice. This amends the historical no-crew-death invariant with owner approval; the invariant against idle/offline/unchosen death stays absolute.
