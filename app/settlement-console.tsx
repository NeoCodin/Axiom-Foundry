...
function SettlementConsole({
  ...
  const [selectedNetworkWorldId, setSelectedNetworkWorldId] = useState<CampaignWorldId | null>(
    colonies[0]?.worldId ?? null,
  );
  ...
  return (
    <section className={`continuity-console settlement-console settlement-console-v2 is-continuity-${activeSection}`} aria-labelledby="settlement-console-title">
      <header className="continuity-console-header">
        <div>
          <p>PLANETARY CONTINUITY · CHAPTER {world.chapter}</p>
      ...
      <nav className="continuity-workspace-tabs" aria-label="Continuity stations">
        <button type="button" className={activeSection === "status" ? "is-active" : ""} aria-pressed={activeSection === "status"} onClick={() => setActiveSection("status")}><span>1</span><strong>World Status</strong><small>{forecast.score}% VIABLE</small></button>
        <button type="button" className={activeSection === "requirements" ? "is-active" : ""} aria-pressed={activeSection === "requirements"} disabled={!requirementsUnlocked} onClick={() => setActiveSection("requirements")}><span>2</span><strong>Requirements</strong><small>{requirementsUnlocked ? `${forecast.deficits.length} REMAIN` : "NOT YET RELEVANT"}</small></button>
        <button type="button" className={activeSection === "works" ? "is-active" : ""} aria-pressed={activeSection === "works"} disabled={!worksUnlocked} onClick={() => setActiveSection("works")}><span>3</span><strong>Planetary Works</strong><small>{worksUnlocked ? "INFRASTRUCTURE" : "LOCKED"}</small></button>
        <button type="button" className={activeSection === "founders" ? "is-active" : ""} aria-pressed={activeSection === "founders"} disabled={!foundersUnlocked} onClick={() => setActiveSection("founders")}><span>4</span><strong>Founding Community</strong><small>{foundersUnlocked ? `${selected.size} SELECTED` : "LATER"}</small></button>
        <button type="button" className={activeSection === "legacy" ? "is-active" : ""} aria-pressed={activeSection === "legacy"} disabled={!legacyUnlocked} onClick={() => setActiveSection("legacy")}><span>5</span><strong>Restored Worlds</strong><small>{legacyUnlocked ? `${colonies.length} RELAYS` : "NO RELAYS"}</small></button>
      </nav>
      ...
      {pelagosSequence && (
        <section className="continuity-panel pelagos-signal-sequence" data-guide-target="pelagos-signal-sequence" aria-labelledby="pelagos-signal-sequence-title">
          ...
          <div className="pelagos-sequence-preview">
            <span className={pelagosSequence.stepNumber > 1 ? "is-complete" : "is-active"}>1 Receiver</span>
            <span className={pelagosSequence.stepNumber > 2 ? "is-complete" : pelagosSequence.stepNumber === 2 ? "is-active" : ""}>2 Habitat</span>
            <span className={pelagosSequence.stepNumber > 3 ? "is-complete" : pelagosSequence.stepNumber === 3 ? "is-active" : ""}>3 Broadcast</span>
            <span className={pelagosSequence.stepNumber > 4 ? "is-complete" : pelagosSequence.stepNumber === 4 ? "is-active" : ""}>4 Listen</span>
            <span className={pelagosSequence.stepNumber === 5 ? "is-active" : ""}>5 Rescue</span>
          </div>
      ...
      <div className="settlement-layout continuity-requirements-workspace">
        <section className="continuity-panel" data-guide-target="planet-requirements">
          ...
          <div className="continuity-readiness-overview">
            ...
            <p>The requirement cards below show exactly what is secured and what still needs work.</p>
          </div>
          <details className="continuity-requirement-ledger">
            <summary><span>ALL REQUIREMENTS</span><strong>Review all {forecast.lines.length} calculations</strong></summary>
        ...
        <section className="continuity-panel" data-guide-target="planet-deficits">
          <header><div><span>ACTION LIST</span><h3>What this world still needs</h3></div><small>Every blocker has a direct next action</small></header>
        ...
        <section className="continuity-panel" data-guide-target="planet-world-works">
          ...
          <div className="world-progress-actions">
            {world.infrastructure.map((objective) => {
              ...
              return <article className={`world-progress-action is-infrastructure ${complete ? "is-complete" : ""}`} key={objective.id}><div><strong>{objective.name}</strong><small>{objective.description}</small></div>{complete ? <span>SECURED</span> : <button type="button" disabled={!quote?.canAfford} onClick={() => onCompleteInfrastructure(objective.id)}>{quote?.costLabel ?? "Unavailable"}</button>}</article>;
            })}
            ...
            {world.equipment.map((definition) => {
              ...
              return (
                <article className={`world-progress-action is-equipment ${complete ? "is-complete" : ""}`} key={definition.id}>
                  <div>
            ...
            {world.requiredResearchIds.map((researchId) => {
              ...
              return <article className={`world-progress-action is-research ${complete ? "is-complete" : ""}`} key={researchId}><div><strong>{titleCase(researchId)}</strong><small>Research project</small></div>{complete ? <span>PROVEN</span> : <button type="button" onClick={onOpenResearch}>Open Research</button>}</article>;
            })}
          </div>
        </section>
        ...
        <section className="continuity-panel planetary-defense-panel">
          ...
          <div className="planetary-doctrine-grid">
            {(Object.keys(PLANETARY_DEFENSE_DOCTRINES) as PlanetaryDefenseDoctrine[]).map((id) => {
              ...
              return <button type="button" className={active ? "active" : ""} aria-pressed={active} onClick={() => onPlanetaryDoctrine(id)} key={id}><span>{definition.label}</span><strong>{formatDecimal(definition.upkeepPerColony * 100)}% per world</strong><small>{definition.summary}</small></button>;
            })}
          </div>
          ...
          <nav className="planetary-network-tabs" aria-label="Restored world defense networks">
            {colonies.map((colony) => {
              const network = planetaryDefense.networks[colony.worldId];
              if (!network) return null;
              const active = selectedNetworkColony?.worldId === colony.worldId;
              return (
                <button
                  type="button"
                  className={active ? "is-active" : ""}
                  aria-pressed={active}
                  onClick={() => setSelectedNetworkWorldId(colony.worldId)}
                  key={colony.worldId}
                >
                  <span>{getCampaignWorld(colony.worldId)?.name ?? colony.worldId}</span>
                  <strong>{getPlanetaryNetworkReadiness(network, planetaryDefense.doctrine)}/100 ready</strong>
                </button>
              );
            })}
          </nav>

          {selectedNetworkColony && (() => {
            const network = planetaryDefense.networks[selectedNetworkColony.worldId];
            if (!network) return null;
            const quotes = planetaryDefenseQuotes[selectedNetworkColony.worldId];
            return (
              <div className="planetary-network-list">
                <article>
                  <header><div><span>{getCampaignWorld(selectedNetworkColony.worldId)?.name.toUpperCase()}</span><strong>{selectedNetworkColony.name}</strong></div><b>{getPlanetaryNetworkReadiness(network, planetaryDefense.doctrine)}/100</b></header>
                  {network.recoveryRemainingSeconds > 0 && <p className="network-instability">Temporary instability: +{formatDecimal(network.instability * 100)}% operational load for {Math.ceil(network.recoveryRemainingSeconds / 60)} more minutes.</p>}
                  <div className="planetary-installation-grid">
                    {planetaryInstallationIds.map((installationId) => {
                      ...
                      return <div key={installationId}><span>{definition.name}</span><strong>Level {network.installations[installationId]}/{MAX_PLANETARY_INSTALLATION_LEVEL}</strong><small>{definition.description}</small><button type="button" disabled={!quote?.canBuild} onClick={() => onPlanetaryConstruction(selectedNetworkColony.worldId, installationId)}>{quote?.maxed ? "MAXED" : quote?.busy ? "Another project active" : quote?.costLabel ?? "Unavailable"}</button></div>;
                    })}
                  </div>
                </article>
              </div>
