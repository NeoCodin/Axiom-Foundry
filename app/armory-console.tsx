...
export type ArmoryConsoleProps = {
  ...
  laws: Readonly<Record<ArmoryLawId, { level: number; maxed: boolean; cost: number; canBuy: boolean }>>;
...
export default function ArmoryConsole({ currentWorldName, quotes, activeProject, laws, onCraft, onRepair, onUpgrade, onModification, onBuyLaw, onOpenHelp, onBack }: ArmoryConsoleProps) {
  ...
  return (
    <section className="continuity-console armory-console" aria-labelledby="armory-console-title">
      ...
      <section className="continuity-panel armory-law-panel"><header><div><span>RECALIBRATION LAWS</span><h3>Change how every future pattern is forged</h3></div><small>Axioms alter the Armory&apos;s permanent manufacturing rules—not individual guns.</small></header><div className="armory-law-grid">{(Object.keys(ARMORY_LAWS) as ArmoryLawId[]).map((id) => { const law = ARMORY_LAWS[id]; const quote = laws[id]; return <article key={id}><span>LAW LEVEL {quote.level}/{law.maxLevel}</span><strong>{law.name}</strong><p>{law.description}</p><button type="button" disabled={!quote.canBuy} onClick={() => onBuyLaw(id)}>{quote.maxed ? "LAW COMPLETE" : `${quote.cost} Axioms`}</button></article>; })}</div></section>
